...
def Hmmgene(data, refer_data, sigma=0.05):
    ...
    refer_data["med"] = refer_data.median(axis=1)
    for i in refer_data:
        refer_data[i] = refer_data[i] - refer_data["med"]
    refer_data = refer_data[c_refer]

    data = data.T
    name = [c for c in data]
    data["med"] = data.median(axis=1)
    for i in name:
        data[i] = data[i] - data["med"]
    data = data[name]
    ...
    refer_data = refer_data.T
    name = [c for c in refer_data]
    refer_data["med"] = refer_data.median(axis=1)
    for i in name:
        refer_data[i] = refer_data[i] - refer_data["med"]
    refer_data = refer_data[name]
    refer_data = refer_data.T

    data["med"] = refer_data.median(axis=1)
    for i in c_data:
        data[i] = data[i] - data["med"]
...
def HMMinferCNV(geneexp, pqpoint, k1, k2):
    ...
    model.covars_ = np.array([[0.01], [0.01], [0.01]])
