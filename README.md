...
```bash
k = 1
classify2 = classify.loc[classify[1] == k, :]
col2 = classify2[0].values
cnv2 = copy.deepcopy(cnv[col2])
cnv2 = cnv2 - 1

refer.index = refer["id"]
cnv2 = cnv2.join(refer, how='inner')
cnv2.index = cnv2["name"].values
cnv2 = cnv2[col2]
cnv2 = cnv2[~cnv2.index.duplicated()]

all_result, occ, mut = analyze.mutualstate(cnv)
#Visual results
